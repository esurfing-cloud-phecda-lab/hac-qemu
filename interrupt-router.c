...
static void qemu_io_router_thread_run_rdma(void)
{
    ...
    for (i = 0; i < qemu_nums; i++) {
		...
		// 设置RDMA对应的listen_rsp_files全局变量，以内存形式进行传输
		...
		// 设置RDMA对应的rsp_files全局变量，以内存形式进行传输（例如shutdown_forwding函数）
...
static void connect_io_router_rdma(void)
{
    ...
    for (i = 0; i < qemu_nums; i ++) {
		...
		// 设置req_files全局变量，RDMA_CONNECT对应的host和port连接到RDMA_LISTEN对应的host和port
...
static void connect_io_router_single(int index)
{
    ...
    connect_addr = g_new0(SocketAddressLegacy, 1);
    ...
    *connect_addr->u.inet.data = (InetSocketAddress) {
    ...
    SocketAddress *connect_addr2 = socket_address_flatten(connect_addr);
	...
	// 设置req_files和rsp_files全局变量，网络的读写就是通过每个计算节点对应的index
...
static void *qemu_io_router_thread_run(void *arg)
{
    ...
    listen_addr = g_new0(SocketAddressLegacy, 1);
    ...
    *listen_addr->u.inet.data = (InetSocketAddress) {
    ...
    // 这里只构建了一个连接（可以设置num的值）
    SocketAddress *listen_addr2 = socket_address_flatten(listen_addr);
    if (qio_channel_socket_listen_sync(lioc, listen_addr2, 1, &local_err) < 0) {
        printf("io-router listen error, exit...");
        object_unref(OBJECT(lioc));
        qapi_free_SocketAddress(listen_addr2);
        exit(1);
    }

	// 进入io_router_accept_connection，最终进入到io_router_loop
...
{
    if (local_cpus == current_machine->smp.cpus)
        return;
...
void start_io_router(void)
{
    ...
    if (local_cpus == -1 || local_cpus == current_machine->smp.cpus)
        return;

	// 这里通过local cpu数据和总cpu数目来计算qemu节点的数目
	// 这里蕴含了一个问题，要求每个计算节点上local cpu的数目要保持相同
    qemu_nums = (current_machine->smp.cpus + local_cpus - 1) / local_cpus;
    if (router_hosts_num != qemu_nums) {
        error_report("invalid number of cluster iplist");
        exit(1);
    }
    printf("QEMU nums: %d, Total CPU nums: %d, CPU per QEMU: %d\n", qemu_nums, current_machine->smp.cpus, local_cpus);
	...
	// 创建线程，进入qemu_io_router_thread_run函数，设置listener，以及connect之后的处理函数io_router_loop
