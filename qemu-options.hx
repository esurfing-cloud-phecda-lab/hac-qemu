...
DEF("local-cpu", HAS_ARG, QEMU_OPTION_local_cpu,
    "-local-cpu [[cpus=]n][,start=start][,iplist=<string>]\n"
...
``-local-cpu [[cpus=]n][,start=start][,iplist=<string>]``
    ...
    and remote respectively. CPUs from start to start + n - 1
    are local, while others are remote. iplist is the IP of each node of the
    distributed VM. Currently, the local CPU number on each node must be the same,
    otherwise the guest would not start.
    ``iplist``
ERST

DEF("qemu-kvm-debug-buffer", HAS_ARG, QEMU_OPTION_qemu_kvm_debug_buffer,
    "-qemu-kvm-debug-buffer [[debug-buffer-file=]f]\n"
    "                debug-buffer-file= debug buffer file path\n",
        QEMU_ARCH_ALL)
SRST
``-qemu-kvm-debug-buffer [[debug-buffer-file=]f]``
    -qemu-kvm-debug-buffer
    Specify the debug buffer file for kvm
ERST
