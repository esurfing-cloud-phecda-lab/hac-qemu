...
extern unsigned long membackend_host_addr_list[8];
extern unsigned long membackend_host_size_list[8];
                                   ...
                                   Error **errp)
{
    ...
    qemu_log("host addr allocated from file %s, start at 0x%lx, size 0x%lx\n", mem_path, (long unsigned int)block->host, size);
    int i=0;
    while(i<sizeof(membackend_host_addr_list) / sizeof(membackend_host_addr_list[0]) && membackend_host_addr_list[i] != 0) i++;
    if (i < sizeof(membackend_host_addr_list) / sizeof(membackend_host_addr_list[0])) {
        membackend_host_addr_list[i] = (long unsigned int)block->host;
        membackend_host_size_list[i] = size;
    }
    return block;
}
#endif
                                                ...
                                                hwaddr l, MemoryRegion *mr)
{
    ...
    bool release_lock = false;
    const uint8_t *buf = (uint8_t *)buf_void;
    ...
    for (;;) {
        if (!memory_access_is_direct(mr, true)) {
            ...
            if (kvm_enabled() && local_cpus != current_machine->smp.cpus) {
...
MemTxResult flatview_read_continue(FlatView *fv, hwaddr addr,
                                        MemTxAttrs attrs, void *buf_void,
                        ...
                        bool *is_dsm)
{
    ...
    *plen = flatview_extend_translation(fv, addr, len, mr, xlat,
    ...
    if (is_dsm && kvm_enabled() && local_cpus != current_machine->smp.cpus)
        *is_dsm = true;
    if (kvm_enabled() && dsm_pin && local_cpus != current_machine->smp.cpus) {
        int ret;
                         ...
                         bool is_write, hwaddr access_len, bool dsm_unpin)
{
    if (buffer != bounce.buffer) {
        ...
        if (kvm_enabled() && dsm_unpin && local_cpus != current_machine->smp.cpus) {
            int ret;
            struct kvm_dsm_mempin pin = {
                .write = is_write,
                .unpin = true,
